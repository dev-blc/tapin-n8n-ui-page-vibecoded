...
export const PlotTwists = () => {
  ...
  const { data: charactersData = [], loading: charactersLoading } =
    usePlotTwistCharacters({ showErrorToast: false });
  ...
  const { data: responseOptionsData = [] } = usePlotTwistResponseOptions({
    showErrorToast: false,
  });
  const {
    createQuest,
  ...
  const plotTwistQuests = useMemo(() => {
    ...
    if (plotTwistQuestsResponse) {
      if (
        plotTwistQuestsResponse.plotTwists &&
        Array.isArray(plotTwistQuestsResponse.plotTwists)
      ) {
        ...
        plotTwistQuestsResponse.data &&
        Array.isArray(plotTwistQuestsResponse.data)
      ) {
    ...
    return questsArray.map((quest) => ({
      ...
      character:
        characterNameMap[quest.characterId] || quest.character || "N/A",
  ...
  const alignedPillars = [
    {
      ...
      description: "Building conscious recognition",
    },
    {
      name: "Light (Clarity)",
      color: "bg-yellow-100 text-yellow-800",
      description: "Gaining clear understanding",
    },
    {
      name: "Intention",
      color: "bg-blue-100 text-blue-800",
      description: "Setting purposeful direction",
    },
    {
      name: "Gratitude",
    ...
    },
    {
      ...
      description: "Embracing present moment",
    },
    {
      name: "Expansion",
      color: "bg-purple-100 text-purple-800",
      description: "Growing beyond limits",
    },
    {
      name: "Dedication",
  ...
  const handleCreateQuest = async () => {
    ...
    try {
      ...
      const responseOptionsArray = Array.isArray(questForm.responseOptions)
        ? questForm.responseOptions
        : [];
      const validResponseOptions = responseOptionsArray.filter(
        (opt) => opt && opt.emoji && opt.text
      );
  ...
  const handleEditQuest = (quest) => {
    ...
    if (
      quest.responses &&
      Array.isArray(quest.responses) &&
      quest.responses.length > 0
    ) {
      responseOptions = quest.responses.map((resp, index) => ({
        emoji: resp.responseEmoji || "",
        text: resp.responseText || "",
        level: resp.engagementLevel
          ? resp.engagementLevel.charAt(0).toUpperCase() +
            resp.engagementLevel.slice(1)
          : "High",
      }));
  ...
  const handleUpdateQuest = async () => {
    ...
    try {
      ...
      await updateQuest(
        { id: selectedQuest.id, data: payload },
      ...
      <div className="space-y-6">
        <Tabs defaultValue="quests" className="w-full">
          <TabsList className="w-full h-full flex flex-nowrap overflow-x-auto md:grid md:grid-cols-4 md:overflow-visible">
            <TabsTrigger
              className="shrink-0 px-3 text-xs md:text-sm"
              value="quests"
            >
              Quest Management
            </TabsTrigger>
                    ...
                    {charactersLoading ? (
                      ...
                      characters.map((character) => (
                        <SelectItem
                          ...
                          value={character.name || character.id || ""}
                        >
                          {character.icon || character.emoji || "👤"}{" "}
                          {character.name}
                        </SelectItem>
                      ))
                    )}
            ...
            {charactersLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {characters.map((character) => {
                  ...
                  const characterQuests = plotTwistQuests.filter(
                    (q) => q.character === (character.name || character.id)
                  );
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-7 gap-1">
                            {[1, 2, 3, 4, 5, 6, 7].map((day) => (
                              <div
                                ...
                                title={
                                  questDays.has(day)
                                    ? `Day ${day} quest exists`
                                    : `Day ${day} quest missing`
                                }
                              >
                                {day}
                              </div>
                          ...
                          <div className="text-xs text-muted-foreground">
                            {characterQuests.length} quest
                            {characterQuests.length !== 1 ? "s" : ""} created
                          </div>
                        </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Card className="p-4">
                      <div className="text-center space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Responses indicating full completion or strong
                          resonance
                        </p>
                        <div className="text-xs text-muted-foreground">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setQuestForm({ ...questForm, character: value })
                    }
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={questForm.day}
                    onValueChange={(value) =>
                      setQuestForm({ ...questForm, day: value })
                    }
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setQuestForm({ ...questForm, pillar: value })
                    }
                  >
                    <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setQuestForm({ ...questForm, tier: value })
                  }
                >
                  <SelectTrigger>
        ...
        {selectedQuest && (
          <Dialog
            open={!!selectedQuest}
            onOpenChange={() => setSelectedQuest(null)}
          >
            <DialogContent className="max-w-4xl">
              <DialogHeader>
                <DialogTitle>
                  {selectedQuest.character || "Unknown Character"} - Day{" "}
                  {selectedQuest.day || "N/A"}
                </DialogTitle>
              </DialogHeader>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      <div>
                        ...
                        <span className="ml-2">
                          {selectedQuest.character || "N/A"}
                        </span>
                      </div>
                      <div>
                        <span className="text-sm font-medium">Day:</span>
                        <span className="ml-2">
                          {selectedQuest.day || "N/A"} of 7
                        </span>
                      </div>
                      <div>
                        ...
                        <Badge
                          className={getPillarColor(selectedQuest.pillar)}
                          variant="outline"
                        >
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div>
                        ...
                        <span className="ml-2 font-semibold">
                          {selectedQuest.responseOptions?.length || 0}
                        </span>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    {selectedQuest.responseOptions &&
                      selectedQuest.responseOptions.length > 0 && (
                        <div>
                          ...
                          <div className="space-y-2">
                            {selectedQuest.responseOptions.map(
                              (option, index) => (
                                <div
                                  key={index}
